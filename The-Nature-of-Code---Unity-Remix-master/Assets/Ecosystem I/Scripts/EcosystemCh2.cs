...
{
    List<EcoMover2> movers = new List<EcoMover2>(); // Now we have multiple Movers!
    EcoAttractor a;

    // Start is called before the first frame update
    void Start()
    {
        int numberOfMovers = 4;
        for (int i = 0; i < numberOfMovers; i++)
        {
            ...
            EcoMover2 m = new EcoMover2(Random.Range(1f, 2f), randomVelocity, randomLocation); //Each Mover is initialized randomly.
            movers.Add(m);
        }
        a = new EcoAttractor();
    }
...
public class EcoAttractor
{
    ...
    public EcoAttractor()
    {
        ...
        body.position = new Vector2(Random.Range(-7f, 7f), Random.Range(-7f, 7f));
        // Generate a radius
        radius = 1f;
...
public class EcoMover2
{
    ...
    public EcoMover2(float randomMass, Vector2 initialVelocity, Vector2 initialPosition)
    {
